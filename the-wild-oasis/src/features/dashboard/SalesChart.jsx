...
function SalesChart({bookings, numDays}) {
    ...
    const data = allDates.map(date => ({
        ...
        totalSales: bookings.filter(b => isSameDay(date, new Date(b.created_at))).reduce((acc, cur) => acc + cur.totalPrice, 0),
        extrasSales: bookings.filter(b => isSameDay(date, new Date(b.created_at))).reduce((acc, cur) => acc + cur.extrasPrice, 0)
    }))
